...
const shouldBypassDownloadCache = (url = '') => (
  BYPASS_GITEE_DOWNLOAD_CACHE &&
  REMOTE_ASSET_SOURCE === 'gitee' &&
  String(url).startsWith(`${REMOTE_ASSET_BASE}/`)
)
...
const cacheImage = async (url) => {
  ...
  if (shouldBypassDownloadCache(url)) {
    return url
  }
