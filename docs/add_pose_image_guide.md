...
轮廓图和真人参考图现在统一放在 `static/pose_pairs/customN/`。历史配置里仍出现的 `/static/pose_guides/..._contour.png` 会在 `utils/assets.js` 中映射到 `/static/pose_pairs/..._contour.png`，新姿势不要再创建 `pose_guides` 副本。

`utils/poses.js` 会过滤被移除的编号、把图片路径转成 Gitee 远程地址，并按 `categoryDefinitions` 组织分类。页面基本都消费 `poseTemplates` 和 `poseCategories`，不直接扫描图片目录。

新增姿势的真人图和轮廓图必须发布到 Gitee：`https://gitee.com/blueMug/posture_assets/raw/main/static/...`。本仓库里的 `static/...` 同路径文件只作为生成、校验和上传来源，不应作为线上运行依赖。
...
| `static/pose_pairs/custom106/` | `custom106_r01_g01_contour.png` | active 相机轮廓，和真人参考图放在同一目录便于查看 |
...
4. `utils/assets.js`
   ...
   新增姿势不要加入本地打包白名单；真人图、缩略图、分享图和轮廓图统一走 Gitee。

   注意 `project.config.json` 里 `static/pose_pairs`、`static/pose_thumbs`、`static/gallery_thumbs`、`static/share_images` 等大资源目录都被忽略打包。运行时会通过 `cdnAssetUrl()` 映射到 `utils/assets.js` 里 `REMOTE_ASSET_SOURCE` 指定的远程源，当前默认是 Gitee 的 `https://gitee.com/blueMug/posture_assets/raw/main/static/...`。

   如果某个资源明确不能本地加载，可加入 `REMOTE_ONLY_ASSET_PATHS`。现有例子是 `custom74` 的 demo 和 thumb。

5. `utils/guideImageSizes.js`

   新增 active 轮廓 PNG 的尺寸记录。相机页首帧布局依赖这个表，尺寸和实际 PNG 不一致会导致进入相机时轮廓先跳一下再稳定。

   目前没有生成脚本，需按 PNG 头信息手动补，或者补一个脚本生成。PNG 宽高可以用系统工具查看，例如：

   ```sh
   sips -g pixelWidth -g pixelHeight static/pose_pairs/custom106/custom106_r01_g01_contour.png
   ```
...
6. 更新 `utils/assets.js` 的远程策略，确认新增资源会走 Gitee。
7. 更新 `utils/guideImageSizes.js`，确保 active `guideImage` 的宽高和真实 PNG 一致。
8. 同步把 `static/...` 资源发布到 Gitee 仓库 `blueMug/posture_assets` 对应路径；本仓库里的同路径只是开发和校验来源，不会打进小程序包。
...
- 不要重新创建 `static/pose_guides/..._contour.png`、`static/recommend_guides/...` 或 `static/home_guides/...`。轮廓统一放在 `static/pose_pairs`，旧路径由 `utils/assets.js` 映射。
...
- 走 Gitee 的资源要确认 Gitee 仓库同路径已发布，否则开发时本地存在，线上仍可能 404。
- 首页缩略图也走 Gitee；不要为了首页首屏把新增姿势缩略图重新放进小程序包。
